...
export default async function Dashboard() {
  ...
  const isAdmin = adminOrganizationIds.length > 0;

  const adminOrderItems = isAdmin
    ? await db.query.orderRequest.findMany({
        where: inArray(orderRequest.organizationId, adminOrganizationIds),
        orderBy: [desc(orderRequest.orderedDate), desc(orderRequest.createdAt)],
      })
    : [];
    ...
    <div className="mx-auto flex min-h-screen w-full max-w-6xl flex-col gap-6 px-4 py-10">
      ...
      {isAdmin ? (
        <Button asChild className="w-fit" variant="outline">
          <Link href="#admin-agenda">Admin agenda</Link>
        </Button>
      ) : null}
